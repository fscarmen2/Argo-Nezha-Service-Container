...
E[2]="Curren architecture \$(uname -m) is not supported. Feedback: [https://github.com/fscarmen/argox/issues]"
C[2]="当前架构 \$(uname -m) 暂不支持,问题反馈:[https://github.com/fscarmen/argox/issues]"
E[3]="Input errors up to 5 times.The script is aborted."
C[3]="输入错误达5次,脚本退出"
E[4]="The script must be run as root, you can enter sudo -i and then download and run again. Feedback:[https://github.com/fscarmen/argox/issues]"
C[4]="必须以root方式运行脚本，可以输入 sudo -i 后重新下载运行，问题反馈:[https://github.com/fscarmen/argox/issues]"
...
C[8]="所有依赖已存在，不需要额外安装"
E[9]="Please enter Github username:"
C[9]="请输入 Github 用户名:"
E[10]="Please enter the Client ID of the Oauth2 application:"
C[10]="请输入 Oauth2 应用的 Client ID:"
...
C[17]="请输入 Github PAT:"
E[18]="There are variables that are not set. Installation aborted. Feedback:: [https://github.com/fscarmen/argox/issues]"
C[18]="参数不齐，安装中止，问题反馈:[https://github.com/fscarmen/argox/issues]"
E[19]="Exit"
C[19]="退出"
...
C[21]="开启哪吒面板"
E[22]="Argo authentication message does not match the rules, neither Token nor Json, script exits. Feedback:[https://github.com/fscarmen/argox/issues]"
C[22]="Argo 认证信息不符合规则，既不是 Token，也是不是 Json，脚本退出，问题反馈:[https://github.com/fscarmen/argox/issues]"
...
check_arch() {
  # 判断处理器架构
  case $(uname -m) in
    aarch64|arm64 ) ARGO_ARCH=arm64; DASHBOARD_ARCH=aarch64 ;;
    x86_64|amd64 ) ARGO_ARCH=amd64; DASHBOARD_ARCH=x86_64 ;;
    * ) error " $(text 25) " ;;
  esac
}
...
check_install() {
  ...
  if [ "$STATUS" = "$(text 26)" ]; then
    ...
    { wget -c https://github.com/fscarmen2/Argo-Nezha-Service-Container/releases/download/grpcwebproxy/grpcwebproxy_linux_$ARGO_ARCH.tar.gz -qO- | tar xz -C $TEMP_DIR >/dev/null 2>&1; }&
    { wget -c https://github.com/fscarmen/nezha/raw/master/resource.tar.gz -qO- | tar xz -C $TEMP_DIR >/dev/null 2>&1; }&
  fi
}
...
check_dependencies() {
  ...
  DEPS_CHECK=("wget" "systemctl" "cron" "timedatectl")
  DEPS_INSTALL=("wget" "systemctl" "cron" "timedatectl")
  for ((g=0; g<${#DEPS_CHECK[@]}; g++)); do [ ! $(type -p ${DEPS_CHECK[g]}) ] && [[ ! "${DEPS[@]}" =~ "${DEPS_INSTALL[g]}" ]] && DEPS+=(${DEPS_INSTALL[g]}); done
  if [ "${#DEPS[@]}" -ge 1 ]; then
    info "\n $(text 7) ${DEPS[@]} \n"
    ${PACKAGE_UPDATE[int]} >/dev/null 2>&1
...
install() {
  ...
  # 根据参数生成哪吒服务端配置文件
  [ "$L" = 'C' ] && timedatectl set-timezone Asia/Shanghai && DASHBOARD_LANGUAGE='zh-CN' || DASHBOARD_LAUNGUAGE='en-US'
  cat > ${WORK_DIR}/data/config.yaml << EOF
debug: false
httpport: $WEB_PORT
language: $DASHBOARD_LAUNGUAGE
grpcport: $GRPC_PORT
...
if [ "\$1" = 'start' ]; then
  ...
  nohup ${WORK_DIR}/grpcwebproxy --server_tls_cert_file=${WORK_DIR}/nezha.pem --server_tls_key_file=${WORK_DIR}/nezha.key --server_http_tls_port=443 --backend_addr=localhost:${GRPC_PORT} --backend_tls_noverify --server_http_max_read_timeout=300s --server_http_max_write_timeout=300s >/dev/null 2>&1 &
...
menu_setting() {
  ...
  if [[ ${STATUS} =~ $(text 27)|$(text 28) ]]; then
    [ ${STATUS} = "$(text 28)" ] && OPTION[1]="1.  $(text 20) " || OPTION[1]="2.  $(text 21) " 
    OPTION[2]="2.  $(text 29)"
